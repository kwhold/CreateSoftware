...
This version of the code drives the individual arm joints to a
particular position, based on potentiometer feedback.  It takes
input from the serial console, and is used in the brick-laying
scenario.
*/
...
// YOURS_NEEDS_UPDATE: The first 4 values in these arrays are the
// experimentally determined values for the potentiometer reads corresponding
// to various position of the arm -- aka, where we want to drive the joint
// positions to in order to be ready to pick up the brick, put down a brick, etc.
const int jointPositions_desired[4][5] = {
  {  17, 114,  60, 530, 0 }, // A: zero position
  { 118, 166, 345, 530, 1 }, // B: pick up brick position (all) & put down #1
  { 118, 166, 345, 555, 1 }, // C: put down #2
  { 118, 166, 345, 580, 1 }, // D: put down #3
};
...
// arm, but you can experiment here.  In particular, if you're using
// new/different potentiometers, you may want to run the motors at slow
// speed as the default first, to get an idea of their range when read
// through the Arduino's analog inputs
// Note: motor speed range: 0-255
const uint8_t slowSpeed   =  50;
const uint8_t normalSpeed = 120;
const uint8_t defaultSpeed = normalSpeed; 
...
void driveAllMotorsToPos(int desiredPosArray) {
  if (jointPositions_desired[desiredPosArray][TURRET_GOES_FIRST] == 1) {
      driveMotorToPos(TURRET_INDEX, desiredPosArray);
  }
  driveMotorToPos(ELBOW2_INDEX, desiredPosArray);
  driveMotorToPos(ELBOW1_INDEX, desiredPosArray);
  driveMotorToPos(SHOULDER_INDEX, desiredPosArray);
  if (jointPositions_desired[desiredPosArray][TURRET_GOES_FIRST] != 1) {
    driveMotorToPos(TURRET_INDEX, desiredPosArray);
  }
}
...
void driveMotorToPos(int motorIdx, int desiredPosArray) {
  ...
  // trouble
  int loopCount = 0;
  while (loopCount < 15 * 10) { // give it 15 seconds
    loopCount = loopCount + 1;
    int currJointPos = readFeedback(motorIdx);
    uint8_t direction = determineMotorDirection(motorIdx, currJointPos, desrdJointPos);
    if (direction == RELEASE) { // we're there
      currMotor->setSpeed(0);
      currMotor->run(RELEASE);
      return;
    }
    if (abs(currJointPos - desrdJointPos) < abs(epsilon * 3)) {
      currMotor->setSpeed(slowSpeed);
    } else {
      currMotor->setSpeed(defaultSpeed);
    }
  ...
  while (incomingByte != 13) { // 13 is the ASCII code for "enter" (aka carriage return)
    if (Serial.available() > 0) {   
      ...
      switch (incomingByte) {
        ...
        case  68: // ascii "D"
        case 100: // ascii "d"
          currentDesiredJointsIndex = 3;
          Serial.println("Setting joint selection to three set.");
...
void setup() {
  ...
  Serial.println("Robot arm brick-layers scenario!");
  Serial.println("Enter A for the first (joint-zero position), B for second (pick up brick),");
  Serial.println("      C for the third (put down #2) and D for the fourth (put down #3).");
  Serial.println("Then hit return for next motion.");
  Serial.println("Example:   A<enter>");
...
void loop() {
  ...
  driveAllMotorsToPos(currentDesiredJointsIndex);
  stopMotors();
}
